...
| 이름  | 역할    | 담당                      |
|-----|----------|-------------------------|
| 최길중 | 관리자 API  | 관리자 CRUD 및 내 프로필 관련 API 개발 |
| 박영수 | 인증/인가 (로그인) | 관리자 (회원가입, 인증, 정보관리) 기능 개발 |
| 박소영 | 고객 API   | 고객 CRUD 개발              |
| 김소현 | 주문 / 대시보드 API | 주문 상태변경, 주문 취소, 대시보드 구현 |
| 홍성현 | 주문 API   | 주문 생성, 주문 조회 기능 구현      |
| 소수경 | 상품/리뷰 API | 상품 CRUD / 리뷰 CRD 기능 구현  |
...
👉 **[Customer Search API - QueryDSL & Paging 설계 학습 정리](docs/TroubleShooting/Customer_QueryDSL.md)**
